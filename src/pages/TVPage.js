...
import { apiKey, fetcher, tmdbApiTVshows } from "../config";
...
const TVPage = () => {
  ...
  const [url, setUrl] = useState(tmdbApiTVshows.getTVs(nextPage));
  ...
  useEffect(() => {
    if (queryDebounce) {
      setUrl(
        `https://api.themoviedb.org/3/search/tv?api_key=${apiKey}&language=en-US&page=${nextPage}&query=${queryDebounce}&include_adult=false`
      );
    } else {
      setUrl(
        `https://api.themoviedb.org/3/tv/popular?api_key=${apiKey}&language=en-US&page=${nextPage}`
      );
    }
  ...
  return (
    <div className="py-10   page-container">
      <div className="flex relative">
        ...
        <button className="absolute right-0 top-[50%] -translate-y-[50%] bg-primary font-semibold h-full w-[50px] flex justify-center items-center rounded-lg text-white">
          <svg
      ...
      {!loading && (
        <div className="gap-5 grid grid-cols-4 my-5">
          {tvshows.length > 0 &&
            tvshows.map((tv) => (
              <MovieCard key={tv.id} item={tv} option="tv"></MovieCard>
            ))}
        </div>
      )}
